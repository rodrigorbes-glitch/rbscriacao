  ...
  useEffect(() => {
    const loadStorefrontData = async () => {
      try {
        ...
        let vitrineProds = prods.filter(p => p.estoque_central > 0);
        setProdutos(vitrineProds);
      ...
      <main style={{ flex: 1, paddingBottom: '4rem', minWidth: 0, width: '100%', overflowX: 'hidden' }}>
        ...
        {produtos.length === 0 ? (
          ...
          <>
            ...
            {/* BARRA DE FILTROS */}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem', backgroundColor: 'var(--color-bg-secondary)', padding: '1.5rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-border)', alignItems: 'center' }}>
